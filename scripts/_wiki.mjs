...
export function parsePlayer(text) {
  /*
   * 生年月日の書き方は記事ごとに違う。次の3つを見てきた。
   *   {{生年月日と年齢|2001|6|4}}
   *   1993年02月09日
   *   [[1998年]][[6月16日]]（28歳）
   * 3つ目はリンク記法が年と月のあいだに挟まるため、素直に読むと拾えない。
   * これを取りこぼすと選手が丸ごと掲載から落ちる（堂安律で実際に起きた）ので、
   * テンプレート以外の形はリンクの括弧を外してから読む。
   */
  const birthField = templateField(text, "生年月日") ?? "";
  const birthPlain = birthField.replace(/\[\[|\]\]/g, "");
  const birth =
    birthField.match(/\{\{生年月日と年齢\|(\d{4})\|(\d{1,2})\|(\d{1,2})/) ??
    birthPlain.match(/(\d{4})年\s*(\d{1,2})月\s*(\d{1,2})日/) ??
