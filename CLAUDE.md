...
1. **本文にURLを入れるかは、投稿の仕方で変える**。課金されるのはAPIから出したときだけで、
   URLを含む投稿は $0.200、含まない投稿は $0.015 と13倍違う。
   Xの画面から手で貼るぶんには料金が発生しないので、そのときはリンクを入れる。
   自動投稿（`scripts/post-to-x.mjs`）は入れない。誘導はプロフィール欄のリンクに任せる。
...
手で投稿するあいだは、下書きを見るページを使う。検索には載せず、どこからも辿れない。

```
https://kaigai-gumi.com/x-drafts/
```

文面は**開いている時刻でブラウザ側が組み立てる**。サイトは6時間ごとにしか作り直さないので、
作った時刻のまま出すと朝に見たとき「今夜」が昨夜を指してしまう。

文面を作る処理は2か所にある。ページは `src/lib/x-draft.ts`、自動投稿は
`scripts/post-to-x.mjs`。後者はGitHub Actionsから動かすため素のJavaScriptで書いてあり、
生成物のファイルを直に読む。**決まりを変えるときは両方を直すこと。**
